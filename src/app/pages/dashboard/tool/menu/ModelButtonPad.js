...
export default class ModelButtonPad extends ModelBase {
  constructor(
    where,
    observers = {},
    { type, code, name, html, id, isIcon, label }
  ) {
    ...
    const boxWrapper = `<li class="toolbar__item toolbar__item--submenu ${
      this.code ? 'toolbar__item--' + this.code : ''
        ...
        <span class="toolbar__label toolbar__label--left">${
          this.name
        }</span></button><menu class="toolbar toolbar--textAlign">
  ...
  updateIcon(data) {
    this.$dom
      .find('.toolbar__button:first')
      .removeClass('toolbar__button--' + this.code)
      .addClass('toolbar__button--' + data.code);
