...
const FontSize = [10, 12, 18, 20, 22, 24, 28, 30, 36, 48, 52, 58, 62, 72, 80, 90, 100, 110, 120, 130];
const FontList = [
    {
        family: "Aileron Thin",
        ...
        i: {
            ...
            w: "https://static.canva.com/static/truetypefonts/Aileron-LightItalic.ttf"
        },
        o: {
            w: {
    ...
    },
    {
        ...
        O: {
            url: "https://static.canva.com/static/images/fonts/Alegreya-Small-Caps.png",
            width: 138,
            height: 35
        },
        W: {
...
export default class ModelList extends ModelBase {
    constructor(where, observers = {}, { type, code, name, html, id, isIcon, label, kind }) {
    ...
    createHTML() {
        ...
        <span class="toolbar__label toolbar__label--left">${this.name}</span></button><menu class="menuList">
    ...
    addBatchItem() {
        if (this.kind === 'fontFamily') {
            FontList.forEach(font => {
                this.addItem(new ModelItemFontFamily(this.$dom, { click: this.callObserver() }, { code: 'fontFamily', name: font.fontFamily, label: font.displayName, isIcon: true, data: font }));
            })
        } else {
            FontSize.forEach(size => {
                this.addItem(new ModelItemFontSize(this.$dom, { click: this.callObserver() }, { isActive: size === 28 ? true : false, code: 'fontSize', name: size, label: size + 'px', isIcon: false, data: size }));
            })
        }
