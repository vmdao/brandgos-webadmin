...
export default class Menu {
  ...
  create() {
    ...
    switch (this.type) {
      ...
      case 'textsvg':
        return this.createToolbarTextSvg();
      default:
    }
  }
  ...
  createToolbarImage() {
    ...
    if (typeof this.observers.listenerTranparency === 'function') {
      ...
      toolbarTranparency.addItem(
        new ModelSlider(
          toolbarTranparency.$dom,
          { slider: toolbarTranparency.callObserver() },
          {
            name: 'Transparency',
            min: 0,
            max: 100,
            step: 1,
            valueDefault: style.opacity * 100,
          }
        )
      );
    ...
    if (typeof this.observers.listenerCopy === 'function') {
      const toolbarCopy = new ModelButton(
        ulElem,
        {
          click: (data) => {
    ...
    if (typeof this.observers.listenerDelete === 'function') {
      const toolbarDelete = new ModelButton(
        ulElem,
        {
          click: (data) => {
            this.remove();
  ...
  createToolbarSvg() {
    ...
    if (typeof this.observers.listenerColor1 === 'function' && style.color1) {
      const toolbarColor = new ModelButtonPad(
        ulElem,
        {
          listener: (data) => {
      ...
      toolbarColor.addItem(
        new ModelColor(
          toolbarColor.$dom,
          { onChangeCompete: toolbarColor.callObserver() },
          style.color
        )
      );
  ...
  createToolbarText() {
    ...
    if (typeof this.observers.listenerFontFamily === 'function') {
      const toolbarFontFamily = new ModelList(
        ulElem,
        {
          listener: (data) => {
    ...
    if (typeof this.observers.listenerFontSize === 'function') {
      const toolbarFontSize = new ModelList(
        ulElem,
        {
          listener: (data) => {
    ...
    if (typeof this.observers.listenerTextSpacing === 'object') {
      const toolbarTextSpacing = new ModelButtonPad(
        ulElem,
        {
          listener: (data) => {
            if (
              typeof this.observers.listenerTextSpacing.listenerLineHeight ===
                'function' &&
              data.name === 'Line height'
            ) {
              return this.observers.listenerTextSpacing.listenerLineHeight(
                this.dataElement,
                data
              );
            }
            ...
            if (
              typeof this.observers.listenerTextSpacing
                .listenerLetterSpacing === 'function' &&
              data.name === 'Letter spacing'
            ) {
              return this.observers.listenerTextSpacing.listenerLetterSpacing(
                this.dataElement,
                data
              );
            }
      ...
      toolbarTextSpacing.addItem(
        new ModelSlider(
          ...
          { slider: toolbarTextSpacing.callObserver() },
          {
            ...
            max: 800,
            step: 10,
            valueDefault: style.letterSpacing,
          }
        )
      );
    ...
    if (typeof this.observers.listenerUppercase === 'function') {
      const toolbarUppercase = new ModelButton(
        ulElem,
        {
          click: (data) => {
            this.hideSubmenu();
  ...
  hideSubmenu() {
    this.$dom
      .find('.toolbar__item--submenuExpanded')
      .removeClass('toolbar__item--submenuExpanded');
  }
