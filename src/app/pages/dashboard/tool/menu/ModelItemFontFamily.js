...
export default class ModelItem extends ModelBase {
  constructor(
    where,
    observers = {},
    { code, name, html, id, isActive, isIcon, label, data }
  ) {
  ...
  createHTML() {
    const boxWrapper = `<li class="selectable ${
      this.isActive ? 'active' : ''
    }"> 
        <button class="toolbar__button toolbar__button--font}">
        <img class="toolbar__thumbnail" src="${this.data.O.url}" width="${
      this.data.O.width
    }" height="${this.data.O.height}"/></button> </li>`;
