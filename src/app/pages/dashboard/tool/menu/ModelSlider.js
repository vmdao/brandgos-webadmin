...
export default class ModelSlider extends ModelBase {
    ...
    createHTML() {
        ...
        <label class="toolbar__label toolbar__label--letterSpacing">${this.name}</label>
        ...
        <div class="ant-slider-handle" style="position: absolute; margin-left: -7px;margin-top: -5px;width: 14px;height: 14px;cursor: pointer;border-radius: 50%;border: 2px solid #88c7f4;
         background-color: #fff; z-index: 2;-webkit-transition: border-color .3s ease,-webkit-transform .3s cubic-bezier(.18,.89,.32,1.28);transition: border-color .3s ease,-webkit-transform .3s cubic-bezier(.18,.89,.32,1.28);transition: border-color .3s ease,transform .3s cubic-bezier(.18,.89,.32,1.28);transition: border-color .3s ease,transform .3s cubic-bezier(.18,.89,.32,1.28),-webkit-transform .3s cubic-bezier(.18,.89,.32,1.28); left:${this.getPercentByValue(this.valueDefault)}%"></div>
        <div class="ant-slider-mark" style="position: absolute;top: 10px;left: 0;width: 100%;font-size: 12px;z-index: 3;"></div></div></div>
        <input class="toolbar__sliderValue toolbar__sliderValue--${this.name}" type="number" min="${this.min}" max="${this.max}" step="${this.step}" value="${this.valueDefault}" disabled="disabled">
        </li>`;
    ...
    onChangeCompete(data) {
        this.data = data;
        typeof this.observers.slider === 'function' ? this.observers.slider(this) : () => { };
    }
