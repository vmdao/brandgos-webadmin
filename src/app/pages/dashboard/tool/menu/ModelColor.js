...
export default class ColorModel {
  ...
  onChangeCompete(data) {
    typeof this.observers.onChangeCompete === 'function' &&
      this.observers.onChangeCompete(data);
  }
  ...
  getHtmlOption() {
    const phantonColorHtml = this.phantonColor.reduce((html, next) => {
      return (
        html +
        `<span><div style="width: 24px; height: 24px;"><div class="option-phanton-color" data-color="${next}" style="background: ${next}; height: 100%; width: 100%; cursor: pointer;"></div></div></span>`
      );
    }, '');
